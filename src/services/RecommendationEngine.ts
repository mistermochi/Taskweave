...
export class RecommendationEngine {
  ...
  async recalibrateFromHistory(allTasks: TaskEntity[], allVitals: UserVital[]): Promise<number> {
    ...
    for (let i = 0; i < completedTasks.length; i++) {
      ...
      // Bolt ⚡: Consolidate activeTasksAtTime creation into a single pass to avoid O(N) filter
      // while preserving Map iteration order and snapshot isolation.
      ...
      const context: SuggestionContext = {
        ...
        tags: [],
        completedTasks: [...previousCompletions], // Bolt ⚡: Maintain snapshot isolation
