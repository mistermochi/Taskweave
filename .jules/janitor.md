...
## 2026-06-20 - Removed Redundant EnvironmentContext
**Clutter:** The `EnvironmentContext` and its provider were redundant wrappers around `process.env.NODE_ENV`.
**Learning:** Next.js makes `process.env.NODE_ENV` available in both client and server components. Creating a React Context for static environment flags adds unnecessary provider nesting and mental overhead. Direct access to `process.env.NODE_ENV` is more idiomatic and efficient for this purpose.
**Action:** Deleted `EnvironmentContext.tsx`, removed the provider from `DataProviders.tsx`, and updated documentation.
