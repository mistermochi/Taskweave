...
export const useEnergyModel = (): EnergyModel => {
  ...
  const model = useMemo(() => {
    ...
    // Bolt ⚡ Optimization: Use .find() for O(1) early-exit and reduced array allocation.
    // VitalsContext already provides data sorted desc, so the first match is the latest.
