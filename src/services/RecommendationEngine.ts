...
export class RecommendationEngine {
  ...
  async recalibrateFromHistory(allTasks: TaskEntity[], allVitals: UserVital[]): Promise<number> {
    ...
    for (let i = 0; i < completedTasks.length; i++) {
      ...
      // Bolt ⚡ Optimization: Construct activeTasksAtTime in a single forEach pass
      // to avoid double array allocation from Array.from().filter().
   ...
   * Bolt ⚡ Optimization: Replace .reduce() with a standard for loop to eliminate
   * callback execution overhead during history replay and suggestion generation.
   */
  private getLatestCompletedTask(tasks: TaskEntity[]): TaskEntity | null {
    ...
    for (let i = 1; i < tasks.length; i++) {
        const current = tasks[i];
        if ((current.completedAt || 0) > (latest.completedAt || 0)) {
            latest = current;
        }
    }
    return latest;
