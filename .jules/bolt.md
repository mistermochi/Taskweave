...
## 2026-04-28 - [O(1) Allocation in History Replay]
**Learning:** The use of `Array.from(iterator).filter()` inside high-frequency loops (like RecommendationEngine history recalibration) causes double array allocation per iteration. Due to the `target: "es5"` constraint, standard `for...of` on iterators is unavailable, but `Map.forEach` provides a zero-allocation way to traverse and filter into a pre-allocated result array.
**Action:** Use `Map.forEach` or standard `for` loops instead of combining iterator-to-array conversions with functional filters in performance-critical paths.
