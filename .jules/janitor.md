...
## 2026-06-25 - Removed Redundant EnvironmentContext
**Clutter:** The redundant `EnvironmentContext` (src/context/EnvironmentContext.tsx) and its provider in `DataProviders.tsx`.
**Learning:** The application strictly prefers using `process.env.NODE_ENV` directly for environment-specific logic (e.g., `isDevelopment`). Using a React Context for static environment flags adds unnecessary nesting and mental overhead in a Next.js application where these flags are globally available.
**Action:** Deleted the unreferenced context file, removed the provider wrapper, and surgically updated `architecture.md` to keep documentation in sync. Verified with a full build, lint, and test sweep.
